    ...
    for sheet in all_sheet:
        ...
        for each_row in range(sheet.nrows):  # 循环打印每一行
            ...
            if each_row != 0:
                ...
                result_map_value = str(col_answer01) + str(col_answer02)
...
def test_risk_questionnaire(data):  # 提交风险测评问卷
    url = 'http://test.yetancaijing.com/eos-api/v1/assetAllocation/risk/questionnaire'
...
def test_questionnaire(post_data):  # 提交诊断问卷
    url = "http://test.yetancaijing.com/eos-api/v1/assetAllocation/ratio/questionnaire"
...
def test_question():  # （获取题库）
    url = "http://test.yetancaijing.com/eos-api/v1/assetAllocation/risk/question"
    get_data = {
...
def test_assetAllocation():  # 获取用户的风险测评结果
    url = "http://test.yetancaijing.com/eos-api/v1/assetAllocation/risk/result"
    get_data = {
        "questionnaireId": 500001  # 问卷ID
    }
    ...
    result_map_value = str(data2['data']["analysisReport"]) + str(data2['data']["maskInfo"])

    return result_map_value


def test_compositeResult():  # 查询资产配置的风险评测和诊断结果（复合接口）
    url = "http://test.yetancaijing.com/eos-api/v1/assetAllocation/ratio/compositeResult"
...
if __name__ == '__main__':
    ...
    # question2(na1)
    quest(ques1, ques2, ques3, ques4, ques5)
    # test_risk_questionnaire(data)   # 提交风险测评问卷
    # test_questionnaire()   # 提交诊断问卷
    # test_question()
